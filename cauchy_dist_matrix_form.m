...
for i=1:length(omega)
    ...
    pdf_val_matrix_init(i) = 10000*params.a/(pi*(norm(omega(i)*eye(length(L_init_eps)) - L_init_eps^(1/2),"fro")^2+params.a^2));
    pdf_val_matrix_final(i) = 10000*params.a/(pi*(norm(omega(i)*eye(length(L_final_eps)) - L_final_eps^(1/2),"fro")^2+params.a^2));
